...
* start ttyd with options:  
  ...
  * `-c` request (basic auth) credentials on connect (replace `<user>`:`<password>` with your own **secure** credentials)
    * be aware that these are basic auth credentials (`.htaccess`) and not *system-user* credentials!
...
> 1. [systemd service](https://linuxvox.com/blog/automatically-run-a-program-on-startup-under-linux-ubuntu/#method-1-using-systemd-recommended-for-services), as root 
...
* edit root crontab: `sudo crontab -e`
* add crontab `@reboot su - <USER> /home/<USER>/bin/StartTTYD.sh` 
  * replace `<USER>` with your system user name,
  * full path to script is required
